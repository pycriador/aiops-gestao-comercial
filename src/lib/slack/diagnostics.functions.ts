...
const canViewSlackDiagnostics = async (userId: string) => {
...
export const getSlackDiagnostics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    if (!(await canViewSlackDiagnostics(context.userId))) throw new Error("Unauthorized");
...
export const testSlackCommandsEndpoint = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const body = new URLSearchParams({
      ...
      team_domain: "diagnostic",
      channel_id: "C_DIAGNOSTIC",
      channel_name: "diagnostic",
